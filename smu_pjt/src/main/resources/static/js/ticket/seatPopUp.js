...
//        if (!scheduleNo) return;
	...
	// ✅ 서버에서 좌석 정보 불러오는 함수 (수정전) - 이게 지금 되는건지안되는건지 모르것
...
//        $('#total').text(selectedSeats.length * 132000 + '원'); // 가격 부분 추후 수정
    ...
    $('.nextStepBtn').on('click', function() {
		...
		let selectedSeats = $(".seat.selected").map(function() {
			return $(this).attr("data-no"); // 좌석 고유번호만 전송
	    }).get();
	    ...
	    $.ajax({
	        url: "/ticketShop/paymentPopUp",
	        type: "POST", 
	        contentType: "application/json",
	        data: JSON.stringify({ selectedSeats: selectedSeats}),
	        success: function(response) {
				alert("성공");
	            window.location.href = "/ticketShop/paymentPopUp?scheduleNo="+ selectedScheduleNo;
	        },
	        error: function(xhr) {
	            console.error("결제 페이지로 이동하는 데 실패했습니다!", xhr.responseText);
	        }
	    });
