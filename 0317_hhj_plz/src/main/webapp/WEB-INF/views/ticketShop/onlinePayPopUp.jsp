...
<%-- 	<%= request.getSession().getAttribute("session_id") %>  --%>
	<script>
	    ...
	    $(document).ready(function() {
	        // saleConcertdto.concertPrice 값을 가져옵니다.
	        ...
	        // 사용 가능 리워드 부분에 값 삽입
	        $(".rewardPoints").text(rewardPoints + " pt");
	...
	<div class="container">
        ...
        <div class="price-box">
            ...
            <div class="price-row"><span>사용 가능 리워드</span><span class="rewardPoints">0 pt</span></div>
            <div class="price-row"><span>사용 할 리워드</span>
                <select class="usePoints">
                    <option value="0">포인트사용 안함</option>
                    <option value="${memberDto.member_mileage}">${memberDto.member_mileage} pt</option>
                </select>
            </div>
            <hr>
            <div class="price-row"><strong>총 결제 금액</strong><strong class="finalPrice">₩${saleConcertdto.concertPrice}</strong></div>
            <div class="price-row"><span>지급 예정 리워드</span><span class="pendingRewards">0 pt</span></div>
        </div>
