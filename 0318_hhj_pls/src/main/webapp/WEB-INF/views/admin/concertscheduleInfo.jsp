...
<html dir="ltr" lang="en">
  <head>
    ...
    <script>
		const deleteBtn = () =>{
			if(confirm("${csdto.scheduleNo}번 스케줄 정보를 삭제하시겠습니까?")){
				$.ajax({
					url:"/concertscheduledelete",
					type:"post",	
					data:{"schedule_no":"${csdto.scheduleNo}"},
					success:function(data){
						alert("스케줄 정보를 삭제했습니다.");
						location.href="/concertschedule"
						console.log(data);
					},
					error:function(){
						alert("실패");
              ...
              <div class="ms-auto text-end">
                <nav aria-label="breadcrumb">
                  <ol class="breadcrumb">
                    ...
                    <li class="breadcrumb-item active" aria-current="/concertschedule">
                      스케줄관리
                    </li>
                  </ol>
                </nav>
        ...
        <div class="container-fluid">
          ...
          <div class="row">
            <div class="col-12">
              <div class="card">
                <div class="card-body">
                  <h5 class="card-title">스케줄등록</h5>
                  <div class="table-responsive">
                  	<form action="/concertschedulewrite" name="writeFrm" method="post" enctype="multipart/form-data">
                    <table
                      id="zero_config"
                    ...
                    >
                      ...
                      <tbody>
                        <input type="hidden" name="scheduleNo" value="${csdto.scheduleNo}">
						<tr>
							<th>콘서트 설명</th>
							<td>
							<select name="SaleConcertDto.saleConcertNo">
							    <c:forEach var="saleConcert" items="${list}">
							        <option value="${saleConcert.saleConcertNo}"
							        <c:if test="${saleConcert.saleConcertNo == csdto.saleConcertDto.saleConcertNo}">selected="selected"</c:if>>${saleConcert.saleConcertDesc}</option>
							    </c:forEach>
							</select>
							</td>
						</tr>
						<tr>
						    <th>스케줄 날짜</th>
						    <td>
						        <input type="date" name="scheduleDate" class="title" style="width:200px; height:30px;" value="${csdto.scheduleDate}">
						    </td>
						</tr>
						<tr>
						    <th>스케줄 시작 시간</th>
						    <td>
						        <input type="time" name="scheduleStartTime" style="width:200px; height:30px;" value="${csdto.scheduleStartTime}">
						    </td>
						</tr>
                      </tbody>
                      <tfoot>
					...
					<button type="button" onclick="location.href='/concertschedule'" class="register-button">취소</button>
                  </div>
                </div>
              </div>
            </div>
