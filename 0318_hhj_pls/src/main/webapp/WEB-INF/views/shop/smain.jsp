...
<div class="recommended-artist-container">
  ...
  <div class="artist-scroll-wrapper">
    <c:choose>
      <%-- 필터링된 목록이 있을 경우 해당 목록 사용 --%>
      <c:when test="${not empty filteredList}">
        <c:forEach var="adto" items="${filteredList}">
      ...
      <%-- 필터링된 목록이 없거나 로그인하지 않은 경우 전체 목록 사용 --%>
      <c:otherwise>
...
<c:choose>
  <%-- 필터링된 목록이 있을 경우 해당 목록 사용 --%>
  <c:when test="${not empty filteredList}">
    <c:forEach items="${filteredList}" var="adto">
      <%-- 상품이 없는 경우 표시하지 않음 --%>
    ...
    <%-- filteredList에 상품이 있는 아티스트가 없는 경우 전체 목록 표시 --%>
    <c:if test="${filteredList.stream().allMatch(adto -> empty adto.shopList)}">
